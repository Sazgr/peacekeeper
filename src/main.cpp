...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss, Search_data& sd, bool cutnode) {
    ...
    if (!is_pv && ss->excluded.is_null() && tt_hit && (entry.type == tt_exact || (entry.type == tt_alpha && entry.score <= static_eval) || (entry.type == tt_beta && entry.score >= static_eval)) && abs(entry.score) < 18000) {
