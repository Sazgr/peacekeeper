...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss, Move (*pv_table)[128], Search_data& sd) {
    ...
    if (ss->excluded.is_null()) ss->static_eval = position.static_eval(*sd.nnue);
    ...
    if (ss->excluded.is_null() && entry.type() == tt_exact && entry.full_hash == position.hashkey()) ss->static_eval = entry.score();
