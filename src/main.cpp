                ...
                if (!timer.stopped() && result > alpha) {
                    ...
                    if (alpha >= beta) {
                        if constexpr (history_heuristic) if (stage == stage_quiet) for (int j{0}; j<i; ++j) {
                            move_order.history_edit(movelist[j].piece(), movelist[j].end(), history_bonus(depth + (static_eval <= alpha)), false);
                            move_order.continuation_edit((ss - 2)->move, movelist[j], history_bonus(depth + (static_eval <= alpha)), false);
                            move_order.continuation_edit((ss - 1)->move, movelist[j], history_bonus(depth + (static_eval <= alpha)), false);
                        }
                        if constexpr (history_heuristic) if (bestmove.captured() == 12) {
                            move_order.history_edit(bestmove.piece(), bestmove.end(), history_bonus(depth + (static_eval <= alpha)), true);
                            move_order.continuation_edit((ss - 2)->move, bestmove, history_bonus(depth + (static_eval <= alpha)), true);
                            move_order.continuation_edit((ss - 1)->move, bestmove, history_bonus(depth + (static_eval <= alpha)), true);
                        }
