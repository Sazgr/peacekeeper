...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss, Search_data& sd, bool cutnode) {
    ...
    for (int i{}; i < movelist.size(); ++i) {
        ...
        reduce_this = 0;
        if constexpr (late_move_reductions) if (depth > 2 && move_num > 2 + 2 * is_pv && !in_check) {
            ...
            reduce_this += (alpha != old_alpha && static_eval + 250 < alpha); //reduce more if alpha has been raised and static eval is far below alpha (no idea why this would work)
