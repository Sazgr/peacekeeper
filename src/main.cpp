...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss, Search_data& sd) {
    ...
    for (int i = 0; i < movelist.size(); ++i) {
        ...
        if constexpr (killer_heuristic) {
            if (movelist[i] == move_order.killer_move(ss->ply, 0)) score += 2000;
            if (movelist[i] == move_order.killer_move(ss->ply, 1)) score += 1000;
            if (ss->ply > 2 && movelist[i] == move_order.killer_move(ss->ply - 2, 0)) score += 500;
            if (ss->ply > 2 && movelist[i] == move_order.killer_move(ss->ply - 2, 1)) score += 250;
        }
