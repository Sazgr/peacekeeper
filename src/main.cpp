...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss)
{
    ...
    ++tt_queries;
    if (entry.type() != tt_none && entry.full_hash == position.hashkey()) ++tt_hits;
    if (!is_pv && ss->excluded.is_null() && entry.type() != tt_none && entry.full_hash == position.hashkey() && entry.depth() >= depth && no_mate(entry.score(), entry.score()) && (entry.type() == tt_exact || (entry.type() == tt_alpha && entry.score() <= alpha) || (entry.type() == tt_beta && entry.score() >= beta))) {
        ++tt_cutoffs;
        return entry.score();
    }
    ...
    if (!timer.stopped() && ss->excluded.is_null()) table.insert(position.hashkey(), alpha, ((alpha > old_alpha)?tt_exact:tt_alpha), bestmove, depth);
