...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss, Search_data& sd, bool cutnode) {
    ...
    for (int stage = stage_hash_move; stage != stage_finished; ++stage) { //generating and sorting one stage
        ...
        for (int i{}; i < movelist.size(); ++i) {
            ...
            if (stage == stage_noisy && move_num != 0 && !in_check && !see(position, movelist[i], -see_noisy_constant - see_noisy_linear * depth - see_noisy_quadratic * depth * depth)) continue;
            if (stage == stage_quiet && move_num != 0 && !in_check && !see(position, movelist[i], -see_quiet_constant - see_quiet_linear * depth - see_quiet_quadratic * depth * depth)) continue;
