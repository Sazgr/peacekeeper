...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss, Search_data& sd, bool cutnode) {
    ...
    if constexpr (internal_iterative_reduction) if (depth >= 6 && !hash_move_usable) reduce_all += 1 + (entry.type != tt_none && entry.full_hash == position.hashkey() && entry.depth >= depth);
    //Stage 1 - Hash Move
