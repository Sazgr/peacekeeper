...
int pvs(Position& position, Stop_timer& timer, Hashtable& table, Move_order_tables& move_order, int depth, int alpha, int beta, Search_stack* ss, Search_data& sd, bool cutnode) {
    ...
    if constexpr (static_null_move) if (depth < 6 && !(ss - 1)->move.is_null() && !is_pv && !in_check && ss->excluded.is_null() && beta > -18000 && tt_hit && (entry.type == tt_exact || entry.type == tt_beta) && (entry.score - futility_base - futility_depth_margin * (depth - improving) >= beta)) {
